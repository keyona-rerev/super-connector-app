 ...
 * 2026-04-07 fixes:
 * - Bug 1: After edit-save, drawer re-fetches from Railway to show fresh data
 * - Bug 2: Email indicator badge on cards and list rows
 * - Bug 3: List/Grid toggle in Browse All view
 */
(function () {
  ...
  let allBuckets   = [];
  let crmDisplayMode = 'grid'; // 'grid' or 'list'
...
<div class="modal-overlay" id="crm-bucket-modal">
      ...
      <div id="help-panel">
        ...
        <div class="help-body">
          <div class="help-panel-content active" id="help-contacts">
            ...
            <p>Open any contact card. Use the <strong>Health</strong> and <strong>Activation</strong> dropdowns in the drawer to update those fields instantly. For all other fields, use <strong>✎ Edit All Fields</strong>. After saving, the drawer refreshes from Railway automatically.</p>
            <h4>Email indicator</h4>
            <p>Cards and list rows show a green <strong>✉ email</strong> badge when an email address is detected in the contact's notes, how-we-met, or what-they-offer fields.</p>
            <h4>Card vs List view</h4>
            <p>Use the <strong>⊞ / ☰</strong> toggle in Browse All to switch between card grid and compact list view.</p>
            ...
            <table class="help-field-table">
              ...
              <tr><td>How We Met</td><td>Narrative context — "panel intro", "warm intro via Bryan". Also used for email detection.</td></tr>
              <tr><td>What Building / Need / Offer</td><td>Powers semantic search. The more detail, the better the matches.</td></tr>
            </table>
            <h4>Buckets</h4>
            <p>Buckets are your custom groupings. A contact can be in multiple buckets. Add/remove from the contact drawer.</p>
  ...
  window.crmSetDisplay = function (mode) {
    crmDisplayMode = mode;
    const gBtn = document.getElementById('crm-view-grid-btn');
    const lBtn = document.getElementById('crm-view-list-btn');
    if (gBtn) gBtn.classList.toggle('active', mode === 'grid');
    if (lBtn) lBtn.classList.toggle('active', mode === 'list');
    const grid = document.getElementById('crm-grid');
    if (grid && grid._contacts) renderGrid(grid._contacts, crmMode === 'search');
  };
  ...
  // ── QUICK-SAVE: inline drawer dropdowns ──────────────────────────────────────
  ...
  window.crmSave = async function () {
    ...
    try {
      ...
      if (data.success) {
        ...
        // BUG 1 FIX: re-fetch from Railway so drawer shows the actual saved state
        if (isEdit) {
          try {
