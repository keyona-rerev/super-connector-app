/**
 * Contacts CRM + Activation Hub — Super Connector App v20260407f
 * Fix: bucket list view contact clicks now open drawer correctly
 * renderList detects bucket grid and routes onclick to crmBucketCardClick
 */
(function () {
...
<div class="modal-overlay" id="crm-bucket-modal">
  ...
  // KEY FIX: detect bucket grid and route onclick accordingly
  function renderList(contacts, isPrimary, targetGrid) {
    const grid = targetGrid || document.getElementById('crm-grid');
    if (!grid) return;
    const isBucketGrid = grid.id === 'crm-bucket-grid';
    const clickFn = isBucketGrid ? 'crmBucketCardClick(this)' : 'crmCardClick(this)';
      ...
      <table style="width:100%;border-collapse:collapse">
        ...
        <tbody>
          ${contacts.map((c, i) => {
            ...
            return `<tr data-cidx="${i}" onclick="${clickFn}" style="border-bottom:0.5px solid var(--border-soft);cursor:pointer;transition:background .1s" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background=''">
